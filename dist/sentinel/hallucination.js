...
class HallucinationDetector {
    ...
    checkForHallucination(correlationId, agentId, spanId, input, output) {
        ...
        // Create safety event
        telemetry_1.sentinelTelemetry.createEvent(correlationId, agentId, spanId, 'hallucination', severity, {
            patterns: detectedPatterns,
            confidence,
            details,
     ...
     */
    checkForContextDrift(input, outputText) {
     ...
     * Check for factual inconsistencies
     */
    checkForFactualInconsistencies(text) {
        const issues = [];
        const lowerText = text.toLowerCase();
     ...
     */
    calculateSeverity(patterns, contextDrift, factualIssues) {
        let score = 0;
     ...
     */
    calculateConfidence(patterns, contextDrift, factualIssues) {
        let confidence = 0;
        // Pattern confidence
