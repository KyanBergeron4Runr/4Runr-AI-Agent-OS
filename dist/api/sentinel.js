...
exports.sentinelRoutes = sentinelRoutes;
const telemetry_1 = require("../sentinel/telemetry");
const config_1 = require("../sentinel/config");
const hallucination_1 = require("../sentinel/hallucination");
const injection_1 = require("../sentinel/injection");
const judge_1 = require("../sentinel/judge");
async function sentinelRoutes(server) {
    // GET /sentinel/metrics - Get Sentinel metrics (Developer View)
    server.get('/sentinel/metrics', async (request, reply) => {
        try {
            const telemetryData = telemetry_1.sentinelTelemetry.getAllTelemetryData();
            const config = config_1.sentinelConfig.getConfig();
            const metrics = {
                // Core metrics
    ...
    server.get('/sentinel/telemetry/:correlationId', async (request, reply) => {
        try {
            ...
            const telemetryData = telemetry_1.sentinelTelemetry.getTelemetryData(correlationId);
            return reply.code(200).send({
                success: true,
                data: telemetryData,
                correlationId,
    ...
    server.get('/sentinel/events/stream', async (request, reply) => {
        try {
            ...
            // Add event listener
            telemetry_1.sentinelEvents.on('guardEvent', eventHandler);
            // Handle client disconnect
            request.raw.on('close', () => {
                telemetry_1.sentinelEvents.off('guardEvent', eventHandler);
    ...
    server.post('/sentinel/test/hallucination', async (request, reply) => {
        try {
            const { input, output, correlationId = 'test', agentId = 'test-agent', spanId = 'test-span' } = request.body;
            const check = hallucination_1.hallucinationDetector.checkForHallucination(correlationId, agentId, spanId, input, output);
            return reply.code(200).send({
                success: true,
                data: {
                    check,
    ...
    server.post('/sentinel/test/injection', async (request, reply) => {
        try {
            const { input, correlationId = 'test', agentId = 'test-agent', spanId = 'test-span' } = request.body;
            const check = injection_1.injectionDetector.checkForInjection(correlationId, agentId, spanId, input);
            return reply.code(200).send({
                success: true,
                data: {
                    check,
    ...
    server.post('/sentinel/test/judge', async (request, reply) => {
        try {
            const { output, evidence, correlationId = 'test', agentId = 'test-agent', spanId = 'test-span', promptMetadata = {} } = request.body;
            const judgeResult = await judge_1.judge.judgeOutput(correlationId, agentId, spanId, output, evidence || [], promptMetadata);
            return reply.code(200).send({
                success: true,
                data: {
    ...
    server.put('/sentinel/config', async (request, reply) => {
        try {
            ...
            config_1.sentinelConfig.updateConfig(newConfig);
            return reply.code(200).send({
                success: true,
    ...
    server.post('/sentinel/events/:eventId/resolve', async (request, reply) => {
        try {
            ...
            const { resolvedBy = 'manual' } = request.body;
            telemetry_1.sentinelTelemetry.resolveEvent(eventId, resolvedBy);
            return reply.code(200).send({
                success: true,
