...
const buildServer = async () => {
    ...
    // Register API routes
    await server.register(agents_1.agentRoutes, { prefix: '/api' });
    await server.register(tokens_1.tokenRoutes, { prefix: '/api' });
    await server.register(proxy_1.proxyRoutes, { prefix: '/api' });
    await server.register(policies_1.policyRoutes, { prefix: '/api' });
    await server.register(admin_1.adminRoutes, { prefix: '/api' });
    await server.register(health_1.healthRoutes); // Health routes at root level
    return server;
};
...
if (require.main === module) {
    const start = async () => {
        try {
            const server = await (0, exports.buildServer)();
