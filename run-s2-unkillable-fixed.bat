@echo off
echo ========================================
echo S2 Unkillable Harness - FIXED (120 Minutes)
echo ========================================
echo.
echo Starting S2 Unkillable Harness (Fixed Version)...
echo Duration: 120 minutes (2 hours)
echo Workloads: W1-W6 with chaos events
echo Expected: ~360 runs, 20 SSE, chaos at 45/75/90m
echo.
echo FIXES APPLIED:
echo - Authentication headers (Authorization, X-Workspace-ID, X-Test-Bypass)
echo - Enhanced error logging with HTTP status codes
echo - Workspace plan setup before tests
echo - Improved SSE diagnostics
echo.
echo Chaos Events:
...
node s2-harness-unkillable-fixed.js
