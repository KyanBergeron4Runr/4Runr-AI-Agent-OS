...
import { createValidationMiddleware } from '../../../packages/middleware/validation/validateRequest';
import { createIdempotencyMiddleware, captureIdempotencyResponse } from '../../../packages/middleware/idempotency/idempotency';
import { InMemoryIdempotencyStore } from '../../../packages/adapters/redis/IdempotencyStore';
import { RunCreateInputSchema } from '../../../packages/middleware/validation/schemas/RunCreate.schema';
...
const VALIDATION_ENFORCE = process.env['VALIDATION_ENFORCE'] || 'strict';
const IDEMPOTENCY_ENABLED = process.env['IDEMPOTENCY_ENABLED'] === 'true';
const IDEMPOTENCY_TTL_SECONDS = parseInt(process.env['IDEMPOTENCY_TTL_SECONDS'] || '86400', 10);
...
const idempotencyStore = new InMemoryIdempotencyStore(IDEMPOTENCY_TTL_SECONDS);
...
fastify.post('/api/runs', {
  preHandler: [
    ...
    createValidationMiddleware(RunCreateInputSchema, VALIDATION_ENFORCE),
    // Idempotency middleware (if enabled)
    ...(IDEMPOTENCY_ENABLED ? [createIdempotencyMiddleware(idempotencyStore)] : [])
  ],
  onSend: [
    // Capture idempotency response (if enabled)
    ...(IDEMPOTENCY_ENABLED ? [captureIdempotencyResponse(idempotencyStore)] : [])
  ]
}, async (request, reply) => {
  const body = request.body as any;
...
fastify.get('/api/runs/logs/stream', async (request, reply) => {
  ...
  // Send generic events every 10 seconds
  const eventInterval = setInterval(() => {
    sendEvent({
      type: 'generic_event',
      timestamp: new Date().toISOString(),
      message: 'S2 harness connection active'
    });
  }, 10000);
...
fastify.post('/api/registry/publish', async (request, reply) => {
  const body = request.body as any;
  logger.info('Registry publish', { package: body.package });
...
fastify.post('/api/privacy/toggle', async (request, reply) => {
  const body = request.body as any;
  logger.info('Privacy toggle', { enabled: body.enabled });
  
  return {
    success: true,
    privacy_enabled: body.enabled,
    updated_at: new Date().toISOString()
  };
});

// Diagnostics endpoint for demo runs
fastify.post('/api/diagnostics/emit-demo-run', async (request, reply) => {
  ...
  runs.set(demoRunId, demoRun);
  logger.info('Demo run created', { runId: demoRunId });
  
  // Emit to any waiting SSE connections
  const connections = sseConnections.get(demoRunId);
  if (connections) {
    connections.forEach(connection => {
      connection.write(`event: log\ndata: ${JSON.stringify({
        type: 'demo_run_created',
        runId: demoRunId,
        timestamp: new Date().toISOString()
      })}\n\n`);
      sseMessageCounter++;
...
// SSE test endpoint for diagnostics
...
const start = async () => {
  try {
    ...
    logger.info(`Validation: ${VALIDATION_ENFORCE}`);
    logger.info(`Idempotency: ${IDEMPOTENCY_ENABLED ? 'enabled' : 'disabled'}`);
    if (IDEMPOTENCY_ENABLED) {
      logger.info(`Idempotency TTL: ${IDEMPOTENCY_TTL_SECONDS} seconds`);
    }
  } catch (err) {
