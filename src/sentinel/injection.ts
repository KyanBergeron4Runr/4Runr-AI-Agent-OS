import { sentinelConfig, isInjectionEnabled } from './config'
...
export class InjectionDetector {
  private config = sentinelConfig.getFeatureConfig('injection')

  /**
   * Check for prompt injection attempts in input
   */
  public checkForInjection(
   ...
   */
  private checkForEncodedContent(text: string): {
    detected: boolean
    patterns: string[]
    details: Record<string, any>
  } {
   ...
   */
  private checkForHiddenText(text: string): {
    detected: boolean
    patterns: string[]
    details: Record<string, any>
  } {
   ...
   * Check for role manipulation attempts
   */
  private checkForRoleManipulation(text: string): {
    detected: boolean
    patterns: string[]
    details: Record<string, any>
  } {
  ...
  private calculateSeverity(
    patterns: string[],
    encodedContent: { detected: boolean; patterns: string[] },
    hiddenText: { detected: boolean; patterns: string[] },
    roleManipulation: { detected: boolean; patterns: string[] }
  ): 'low' | 'medium' | 'high' {
    let score = 0
   ...
   */
  private determineAction(severity: 'low' | 'medium' | 'high', confidence: number): 'flag' | 'block' | 'mask' | 'require_approval' {
   ...
   */
  private sanitizeInput(input: any, detectedPatterns: string[]): any {
