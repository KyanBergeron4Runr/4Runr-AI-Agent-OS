...
export function initializeMonitoring(): void {
  ...
  multiLevelMonitor.on('monitoring-started', () => {
  ...
  multiLevelMonitor.on('metrics-collected', (level, data) => {
...
export async function getMonitoringDashboard() {
  ...
  try {
    ...
    const [applicationData, systemData, dockerData, infrastructureData] = await Promise.all([
      multiLevelMonitor.getRecentData('application', 5),
      multiLevelMonitor.getRecentData('system', 5),
      multiLevelMonitor.getRecentData('docker', 5),
      multiLevelMonitor.getRecentData('infrastructure', 5)
    ])
...
export async function getMetricsHistory(options: {
  ...
  return await multiLevelMonitor.queryData({
    level,
    startTime,
    limit
  })
...
export async function getSystemHealthSummary() {
  ...
  try {
    ...
    const [applicationData, systemData, dockerData, infrastructureData] = await Promise.all([
      multiLevelMonitor.getRecentData('application', 1),
    ...
    const health = {
      ...
      application: {
        ...
        metricsCount: latest.application?.success ? Object.keys((latest.application.data as any)?.parsed?.counters || {}).length : 0,
        lastUpdate: latest.application?.timestamp
      },
      system: {
        available: latest.system?.success || false,
        cpuUsage: latest.system?.success ? (latest.system.data as any)?.cpu?.usage : null,
        memoryUsage: latest.system?.success ? (latest.system.data as any)?.memory?.used : null,
        lastUpdate: latest.system?.timestamp
      },
      docker: {
        available: latest.docker?.success || false,
        containerCount: latest.docker?.success ? (latest.docker.data as any)?.containers?.length : 0,
        lastUpdate: latest.docker?.timestamp
      },
      infrastructure: {
        available: latest.infrastructure?.success || false,
        databaseConnected: latest.infrastructure?.success ? (latest.infrastructure.data as any)?.database?.connected : false,
        redisConnected: latest.infrastructure?.success ? (latest.infrastructure.data as any)?.redis?.connected : false,
        lastUpdate: latest.infrastructure?.timestamp
      }
    }

    // Calculate overall health score
    const healthyLevels = Object.values(health).filter(level => 
      typeof level === 'object' && level.available
    ).length - 1 // Subtract 1 for timestamp
...
export async function forceMetricsCollection() {
  ...
  multiLevelMonitor.emit('force-collection')
...
export async function getMonitoringStats() {
  ...
  try {
    ...
    const data = await multiLevelMonitor.queryData({
