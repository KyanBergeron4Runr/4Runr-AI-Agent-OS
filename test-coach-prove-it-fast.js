// Prove-It E2E Test - Coach System (FAST VERSION)
// Tests the complete Coach feedback engine with real A/B experimentation

const http = require('http')

console.log('🎯 Prove-It E2E Test - Coach System (FAST VERSION)')
console.log('==================================================')
...
// Helper to generate test runs with varying inputs (FAST VERSION)
function generateTestInputs(count = 10) {
  const subjects = [
...
// Helper to simulate agent runs and collect metrics (FAST VERSION)
async function simulateAgentRuns(agentId, count = 10, slot = 'A') {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Simulate agent processing with minimal delays (FAST VERSION)
    const startTime = Date.now()
    const processingTime = slot === 'A' ? 
      Math.random() * 100 + 50 : // Slot A: 50-150ms (fast)
      Math.random() * 80 + 40    // Slot B: 40-120ms (faster)
    ...
    // Minimal delay for fast testing
...
async function runProveItTest() {
  ...
  // Test 3: Generate Baseline Data (Slot A) - FAST VERSION
  totalTests++
  console.log('\n3️⃣ Generating Baseline Data (Slot A)...')
  try {
    console.log('   Simulating 10 baseline runs (fast)...')
    const baselineRuns = await simulateAgentRuns(TEST_AGENT_ID, 10, 'A')
  ...
  try {
    const response = await makeRequest('GET', `/api/coach/report/${TEST_AGENT_ID}?runCount=10`)
    if (response.statusCode === 200 && response.body?.success) {
  ...
  // Test 6: Generate Slot B Data (Patched) - FAST VERSION
  totalTests++
  console.log('\n6️⃣ Generating Slot B Data (Patched)...')
  try {
    console.log('   Simulating 10 patched runs (fast)...')
    const patchedRuns = await simulateAgentRuns(TEST_AGENT_ID, 10, 'B')
  ...
  // Final Results
  console.log('\n🎯 Prove-It E2E Test Results (FAST VERSION):')
  console.log('=============================================')
  console.log(`✅ Passed: ${passedTests}/${totalTests}`)
  console.log(`❌ Failed: ${totalTests - passedTests}/${totalTests}`)

  if (passedTests === totalTests) {
    console.log('\n🎉 PROVE-IT E2E TEST PASSED! (FAST VERSION)')
    console.log('============================================')
